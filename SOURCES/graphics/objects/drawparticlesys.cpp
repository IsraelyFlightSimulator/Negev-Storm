...
public:
		...
		void	*GetRandomArgument(void) { return ptr[(int)(PRANDFloatPos() * ( (float)Items - 0.1f ))]; }
		void	*GetArgument(DWORD Index) { return Index<Items? ptr[Index] : NULL; }

		ParticleGroupNode  *Next;
		ParticleGroupNode( char *name) { Items=0; strcpy(Name, name); Next=NULL; Type=GRP_NONE;};
		GroupType	Type;
...
{
public:
	SubPart(ParticleNode *owner) { next = 0; Alive=true; };
	SubPart() { next = 0; Alive=true; };
	virtual ~SubPart() {}
	SubPart *next;
	bool	Alive;
...
SubPartPoly::SubPartPoly(ParticleNode *owner) 
{
	...
	next = 0; AlphaStage=ColorStage=SizeStage=LightStage=0;
			...
			if(n)
			{
				if(ppn=new ParticleParamNode)
				{
					...
					if(i){
						...
						if(Fn) { *ppn=*Fn; paramList.AddTail(ppn); continue; }
					}
...
void DrawableParticleSys::PS_ListInit(DWORD i, DWORD nr, DWORD size)
{
	...
	if (!PS_Lists[i].ObjectList){ PS_Lists[i].ObjectList = calloc(nr, size); }
	if (!PS_Lists[i].IndexList){ PS_Lists[i].IndexList = (PS_PTR*)calloc(nr, sizeof(PS_PTR)); }
	// reset indexes
	for (DWORD n=0; n<nr; n++){ PS_Lists[i].IndexList[n] = (PS_PTR)n; }
	// reset pointers
...
{	
	...
	if(!PS_PPN) { PS_PPN=(PS_PPType*)malloc(MAX_PARTICLE_PARAMETERS * sizeof(PS_PPType)); PS_PPNNr=0; }
	if(!PS_TPN) { PS_TPN=(PS_TPType*)malloc(MAX_TRAIL_PARAMETERS * sizeof(PS_TPType)); PS_TPNNr=0; }
	if(!PS_PEP) { PS_PEP=(PS_PEPType*)malloc(MAX_EMITTERS_PARAMETERS * sizeof(PS_PEPType)); PS_PEPNr=0; }
...
void DrawableParticleSys::PS_ListsRelease(void)
{
	for (DWORD i=0; i<PS_LISTS_NR; i++){
		if (PS_Lists[i].IndexList){ free(PS_Lists[i].IndexList); }
		if (PS_Lists[i].ObjectList){ free(PS_Lists[i].ObjectList); }
		PS_Lists[i].IndexList=NULL;
		PS_Lists[i].ObjectList=NULL;
		PS_Lists[i].ListSize=0;
	}
	if (PS_PPN){ free(PS_PPN); PS_PPN=NULL; }
	if (PS_PEP){ free(PS_PEP); PS_PEP=NULL; }
	if (PS_TPN){ free(PS_TPN); PS_TPN=NULL; }
	PS_SubTrails=0;
}
...
PS_PTR DrawableParticleSys::PS_AddItem(DWORD ListIdx)
{
	...
	if(List.ObjectIn==List.ObjectOut) { List.ObjectIn=LastObj; return PS_NOPTR; }
	
	// here if exists a previous Item to link to
	switch(ListIdx){
		
		case	PS_PARTICLES_IDX :	Item=AddPart(List, (ParticleNodeType*)List.ObjectList, Item);
									break;

		case	PS_POLYS_IDX :		Item=AddPart(List, (PolySubPartType*)List.ObjectList, Item);
									break;

		case	PS_EMITTERS_IDX :	Item=AddPart(List, (EmitterPartType*)List.ObjectList, Item);
									break;

		case	PS_SOUNDS_IDX :		Item=AddPart(List, (SoundSubPartType*)List.ObjectList, Item);
									break;

		case	PS_LIGHTS_IDX :		Item=AddPart(List, (LightPartType*)List.ObjectList, Item);
									break;

		case	PS_TRAILS_IDX :		Item=AddPart(List, (TrailEmitterType*)List.ObjectList, Item);
									break;

		case	PS_CLUSTERS_IDX :	Item=AddPart(List, (ClusterPosType*)List.ObjectList, Item);
									break;
...
inline PS_PTR	RemovePart(PS_ListType	&List, _part_t *Part, PS_PTR Item, PS_PTR Prev)
{
	...
	if(Prev==PS_NOPTR) { List.ListEntry=Part[Item].NEXT; return List.ListEntry; }
	// if in the middle
...
PS_PTR DrawableParticleSys::PS_RemoveItem(DWORD ListIdx, PS_PTR Item, PS_PTR Prev)
{
	...
	switch(ListIdx){
		...
		case	PS_PARTICLES_IDX :	Item=RemovePart(List, (ParticleNodeType*)List.ObjectList, Item, Prev);
									break;

		case	PS_POLYS_IDX :		Item=RemovePart(List, (PolySubPartType*)List.ObjectList, Item, Prev);
									break;

		case	PS_EMITTERS_IDX :	Item=RemovePart(List, (EmitterPartType*)List.ObjectList, Item, Prev);
									break;

		case	PS_SOUNDS_IDX :		Item=RemovePart(List, (SoundSubPartType*)List.ObjectList, Item, Prev);
									break;

		case	PS_LIGHTS_IDX :		Item=RemovePart(List, (LightPartType*)List.ObjectList, Item, Prev);
									break;

		case	PS_TRAILS_IDX :		Item=RemovePart(List, (TrailEmitterType*)List.ObjectList, Item, Prev);
									break;

		case	PS_CLUSTERS_IDX :	Item=RemovePart(List, (ClusterPosType*)List.ObjectList, Item, Prev);
									break;
...
void	 DrawableParticleSys::PS_SoundRun(void)
{
	...
	while(ptr!=PS_NOPTR){
		...
		if(!Part.Alive) { ptr=PS_RemoveItem(PS_SOUNDS_IDX, ptr, LastPtr); continue; }
		// The Life
		...
		if(Sound.Play && PS_ElapsedTime)
		{
			...
			if(!Sound.Looped) { ptr=PS_RemoveItem(PS_SOUNDS_IDX, ptr, LastPtr); continue; }
...
void	 DrawableParticleSys::PS_LightsRun(void)
{
	...
	while(ptr!=PS_NOPTR){
		...
		// If Particle already dead, end here and go to next
		if(!Light.Alive) { ptr=PS_RemoveItem(PS_LIGHTS_IDX, ptr, LastPtr); continue; }
...
void	 DrawableParticleSys::PS_PolyRun(void)
{
	...
	while(ptr!=PS_NOPTR){
		...
		// If Particle already dead, end here and go to next
		if(!Part.Alive) { ptr=PS_RemoveItem(PS_POLYS_IDX, ptr, LastPtr); continue; }
		// The Life
		float Life=Part.life;
		// Pointer to particle parametes
...
void	 DrawableParticleSys::PS_EmitterRun(void)
{
	...
	while(ptr!=PS_NOPTR){
		...
		// If Particle already dead, end here and go to next
		if(!Part.Alive) { ptr=PS_RemoveItem(PS_EMITTERS_IDX, ptr, LastPtr); continue; }
		// The Life
		float Life=Part.life;
		...
		switch(Emitter.PEP->mode){
			...
			case PSEM_IMPACT:	if( epos.z >= Part.GroundLevel ) epos.z = Part.GroundLevel, qty+=Emitter.PEP->rate[0].value, Alive=false;
								break;


			// * EARTH IMPACT * if on ground
			case PSEM_EARTHIMPACT: if( epos.z >= Part.GroundLevel ){
			...
			// * WATER IMPACT *
			case PSEM_WATERIMPACT:if( epos.z >= Part.GroundLevel ){
										epos.z = Part.GroundLevel;
										// get ground type
										int gtype = OTWDriver.GetGroundType(epos.x, epos.y);
										// if on WATER or RIVER add the quantity and die
			...
			// * ONCE EMISSION *
			case PSEM_ONCE:			while ( Emitter.LastStage < Emitter.PEP->stages && (Emitter.PEP->rate[Emitter.LastStage].time + Emitter.RndTime) <= Part.life ){
										// Add quantity and go to next stage
			...
			case PSEM_PERSEC:		qty = PS_EvalTimedFloat(Part.life, Emitter.LastStage, Emitter.PEP->stages, Emitter.PEP->rate);
...
void DrawableParticleSys::PS_GenerateEmitters(PS_PTR owner, PS_PPType &PPN)
{
	...
	if(PPN.emitOneRandomly){
		...
		if(count) { int l = rand() % count; PS_AddEmitter(owner, &PPN.emitter[l], Light); }
	} else {
...
void DrawableParticleSys::PS_ParticleRun(void)
{
	...
	while(ptr!=PS_NOPTR){
		...
		if(!Part.Alive) { ptr=PS_RemoveItem(PS_PARTICLES_IDX, ptr, LastPtr); continue; }

		// Security check, no negative life
		if(Part.lifespan<=0) { Part.Alive=false; goto Skip; }
...
void DrawableParticleSys::PS_ClustersRun(void)
{
	...
	while(ptr!=PS_NOPTR){
		// Get the object
		ClusterPosType &Cluster=(((ClusterPosType*)PS_Lists[PS_CLUSTERS_IDX].ObjectList)[ptr]);
		if(!Cluster.Alive) { ptr=PS_RemoveItem(PS_CLUSTERS_IDX, ptr, LastPtr); continue; }
		LastPtr=ptr;
		ptr=Cluster.NEXT;
...
void	DrawableParticleSys::PS_SubTrailRun(TrailSubPartType *Trail, D3DXVECTOR3 &Origin, DWORD &Entry, DWORD &Exit, DWORD Elements, PS_PTR tpn)
{
	...
	while(!TrailCompleted /*&& PS_SubTrails<14200*/){
		...
		if(Life>= 1.0f)
			{ if(!TrailCompleted) Entry=Index; TrailValid = LineMode = false, RecalcP1 = true; Integrate = 0; goto Skip; }
		...
		{	pan=0;
			...
			if(n)
			{
				...
				if(i){
					...
					if(c<MAX_PARTICLE_PARAMETERS) { PS_PPN[LastPPN]=PS_PPN[c]; ppn=&PS_PPN[LastPPN++]; continue; }
				}
		...
		{	GroupNode=NULL;	
			...
			if(n)
			{
				...
				if(i){
					...
					// if found copy it's values and skip the rest
					if(c<MAX_TRAIL_PARAMETERS) { PS_TPN[LastTPN]=PS_TPN[c]; tpn=&PS_TPN[LastTPN++]; continue; }
				}
				// New PPN, assign to pointer and count one more
				tpn=&PS_TPN[LastTPN++];
				// Default Values
...
/////////////////////////////////////////\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
		...
		if(ppn){														// * PARTICLE PARAMETER NODE *
			...
			On("flags") { ppn->flags = TokenFlags(0,PSF_CHARACTERS); continue; }
			...
			On("drag") { ppn->simpleDrag=TokenF(0); continue; }

			On("bounce") { ppn->bounce=TokenF(0); continue; }
...
/////////////////////////////////////////\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
		...
		if(tpn){														// * TRAIL PARAMETER NODE *
			...
			On("rate") { tpn->Interval=TokenF(1.0f); continue; }
			...
			On("random") { tpn->RndStep = TokenF(0.0f), tpn->RndLimit = TokenF(0.0f), tpn->RndTime = TokenF(0.0f) ;  continue; }
...
/////////////////////////////////////////////////////////////////\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
	...
	for(DWORD a=0; a<LastTPN; a++){
		...
		while(Table->Name[0]){
			...
			if(!strcmp(PS_TPN[a].Name, Table->Name)) { PS_TrailsID[Table->Id]=a; break; }
			Table++;
		}
	}
	...
	for(DWORD c=0; c<MAX_PARTICLE_PARAMETERS; c++){
		...
		if(PS_PPN[c].TrailName[0]){
			...
			for(DWORD a=0; a<LastTPN; a++){
				...
				if(!strcmp(PS_TPN[a].Name, PS_PPN[c].TrailName)) PS_PPN[c].trailId = a;
			}
		}
	}
