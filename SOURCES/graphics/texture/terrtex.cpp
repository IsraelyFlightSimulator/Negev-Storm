...
namespace {
	int getDDSWidth(int flags){
		if (flags & MPR_TI_16){ return 16; }
		else if (flags & MPR_TI_32){ return 32; }
		else if (flags & MPR_TI_64){ return 64; }
		else if (flags & MPR_TI_128){ return 128; }
		else if (flags & MPR_TI_256){ return 256; }
		else if (flags & MPR_TI_512){ return 512; }
		else if (flags & MPR_TI_1024){ return 1024; }
		else if (flags & MPR_TI_2048){ return 2048; }
		else {
			// BUG!!!
			return 4096;
		}
...
bool TextureDB::DumpImageToFile(TileEntry* pTile, DWORD *palette, int res, bool bForce)
{
	...
	if(!fp || bForce || bIs092)
	{
		...
		while(to < stop)
		{
			*to = static_cast<BYTE>(FloatToInt32(*from * 0.f)); to++, from++;	// Red
			*to = static_cast<BYTE>(FloatToInt32(*from * 0.f)); to++, from++;	// Green
			*to = static_cast<BYTE>(FloatToInt32(*from * 0.f)); to++, from++;	// Blue
														 	    to++, from++;	// Alpha
		}
